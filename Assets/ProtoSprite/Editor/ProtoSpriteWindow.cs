...
namespace ProtoSprite.Editor
{
    public class ProtoSpriteWindow : EditorWindow, IHasCustomMenu
    {
		...
		internal int PromptState { get => m_PromptState;
            set
            {
                m_PromptState = value;
                EditorPrefs.SetInt("ProtoSprite.Editor.ProtoSpriteWindow.PromptState", m_PromptState);
            }
        }

		public int TargetTextureIndex { get => m_TargetTextureIndex; set => m_TargetTextureIndex = value; }

        public static Texture2D GetTargetTexture()
        {
            SpriteRenderer spriteRenderer = Selection.activeGameObject.GetComponent<SpriteRenderer>();
            Sprite sprite = spriteRenderer.sprite;
            Texture2D texture = spriteRenderer.sprite.texture;

            if (GetInstance().TargetTextureIndex == 0)
            {
            ...
            SecondarySpriteTexture[] secondarySpriteTextures = textureImporter.secondarySpriteTextures;

            int secondaryTextureIndex = GetInstance().TargetTextureIndex - 1;

            if (secondaryTextureIndex >= secondarySpriteTextures.Length)
            {
                return SpriteUtility.GetSpriteTexture(sprite, false);
            }

            if (secondarySpriteTextures[secondaryTextureIndex].texture == null)
            {
                return SpriteUtility.GetSpriteTexture(sprite, false);
            }

            return secondarySpriteTextures[secondaryTextureIndex].texture;
        }
        ...
        void OnEnable()
        {
            ...
            m_PromptState = EditorPrefs.GetInt("ProtoSprite.Editor.ProtoSpriteWindow.PromptState", 0);
        ...
        void OnBeforeSceneGUI(SceneView sceneView)
        {
            ...
            //Debug.Log(Time.frameCount + " on before scene gui");
            ...
            //sceneView.camera.targetTexture

            Graphics.Blit(sceneRT, tempRT);
            ...
            //GL.Clear(true, true, sceneView.camera.backgroundColor.linear);

            Graphics.CopyTexture(tempRT, sceneRT);
        ...
        void OnSceneGUI(SceneView sceneView)
        {
            ...
            {
                PaintTool.DrawDebugSprite(Selection.activeGameObject.GetComponent<SpriteRenderer>(), GetTargetTexture(), sceneView);
            }
        }
        ...
        private void OnGUI()
        {
            ...
            // Duplicate sprite
            {
                ...
                guiContent.tooltip = "Create a duplicate sprite texture file and set the currently selected SpriteRenderer to reference it. Useful for creating a new frame when animating or for quickly creating a variant. This will also duplicate all linked secondary textures.";
            ...
            if (EditorApplication.timeSinceStartup > (60.0f * 10) && PromptState == 0)
            {
                PromptState = 1;
            }
            ...
            {
                //GUI.enabled = isValid;
                GUIContent guiContent = new GUIContent(EditorGUIUtility.IconContent("Favorite"));
                guiContent.tooltip = "What's this?";
            ...
            if (isValid)
            {
                ...
                {
                    textureNames.Add(secondarySpriteTextures[i].name);
                    //Debug.Log("sst: " + secondarySpriteTextures[i].texture);
                }

                //textureNames.Add("Create New");

                /*var newTextureTargetIndex = EditorGUILayout.Popup(TargetTextureIndex, textureNames.ToArray(), GUILayout.Width(50));

                if (TargetTextureIndex != newTextureTargetIndex && newTextureTargetIndex == textureNames.Count - 1)
                {
                    CreateSecondaryTextureWindow.Open();
                }
                else
                {
                    TargetTextureIndex = newTextureTargetIndex;
                }*/

                {
                    string targetTextureName = TargetTextureIndex < textureNames.Count ? textureNames[TargetTextureIndex] : "              ";

                    var guiContent = new GUIContent(targetTextureName);// new GUIContent(EditorGUIUtility.IconContent("CreateAddNew"));
                    guiContent.tooltip = "Create a new GameObject and associated sprite texture file.";

                    Rect dropPosRect = GUILayoutUtility.GetRect(guiContent, EditorStyles.toolbarDropDown);

                    if (GUI.Button(dropPosRect, guiContent, EditorStyles.toolbarDropDown))
                    {
                        ...
                        {
                            int index = i;
                            toolsMenu.AddItem(new GUIContent(textureNames[index]), targetTextureName == textureNames[index], delegate
                            {
                        ...
                        toolsMenu.AddItem(new GUIContent("Create Secondary Texture"), false, CreateSecondaryTextureWindow.Open);

                        // Offset menu from right of editor window
                        toolsMenu.DropDown(new Rect(dropPosRect.x, 5, 0, 16));
                        EditorGUIUtility.ExitGUI();
                    }
                }
            }
        ...
        void AutoFix()
        {
            ...
            for (int i = 0; i < selectedObjects.Length; i++)
            {
                ...
                if (gameObject != null)
                {
                    ...
                    {
                        Fix(spriteRenderer.sprite.texture, true);
                    }
                }
            }
        }

        void Fix(Texture2D texture, bool includeSecondaryTextures)
        {
            if (texture == null)
            {
                Debug.LogError("ProtoSprite: Trying to AutoFix a NULL texture.");
                return;
            }
            ...
            if (includeSecondaryTextures)
            {
                var secondaryTextures = importer.secondarySpriteTextures;
                foreach (var st in secondaryTextures)
                {
                    Fix(st.texture, false);
                }
            }
        ...
        static void DuplicateSprite(bool extractMultisprite)
        {
            ...
            int targetTextureIndex = ProtoSpriteWindow.GetInstance().TargetTextureIndex;

            TextureImporter textureImporter = (TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(selectedTexture));

            var secondaryTextures = textureImporter.secondarySpriteTextures;

            string newPath = EditorUtility.SaveFilePanelInProject("Create PNG (_MainTex)", assetName, "png", "Enter file name", folderPath);

            if (newPath.Length != 0)
            {
                string finalAssetName = Path.GetFileNameWithoutExtension(newPath);
                string finalFolderPath = Path.GetDirectoryName(newPath);
                ...
                if (extractMultisprite)
                {
                    Texture2D newTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(newPath);
                    CropSpriteTexture(newTexture, selectedSprite.pivot, selectedSprite.rect);
                }

                TextureImporter newTextureImporter = (TextureImporter)AssetImporter.GetAtPath(newPath);
                ...
                for (int i = 0; i < secondaryTextures.Length; i++)
                {
                    ...
                    //string folderSTPath = Path.GetDirectoryName(path);

                    string newSTPath = EditorUtility.SaveFilePanelInProject("Create PNG (" + data.name + ")", finalAssetName + data.name, "png", "Enter file name", finalFolderPath);

                    if (newSTPath.Length != 0)
                    {
                        ...
                        if (extractMultisprite)
                        {
                            Texture2D newTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(newSTPath);
                            CropSpriteTexture(newTexture, selectedSprite.pivot, selectedSprite.rect);
                        }
                    ...
                    {
                        Debug.LogWarning("ProtoSprite: Didn't create secondary texture (" + data.name + ") for: " + newPath, sprite);
                    }
                }
        ...
        /*static void DuplicateSprite()
        {
            ...
            Sprite selectedSprite = spriteRenderer.sprite;
            Texture2D selectedMainTexture = SpriteUtility.GetSpriteTexture(selectedSprite, false);
            Texture2D selectedTexture = GetTargetTexture();// SpriteUtility.GetSpriteTexture(selectedSprite, false);
            string selectedAssetPath = AssetDatabase.GetAssetPath(selectedTexture);
            string assetPath = AssetDatabase.GenerateUniqueAssetPath(selectedAssetPath);
            string assetName = Path.GetFileNameWithoutExtension(assetPath);
            string folderPath = Path.GetDirectoryName(assetPath);
            ...
            bool isMainTexture = targetTextureIndex == 0;

            TextureImporter textureImporter = (TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(selectedMainTexture));

            var secondaryTextures = textureImporter.secondarySpriteTextures;

            if (targetTextureIndex > secondaryTextures.Length)
                isMainTexture = true;
        ...
        static void CropSpriteTexture(Texture2D texture, Vector2 spritePivot, Rect spriteRect)
        {
            string assetPath = AssetDatabase.GetAssetPath(texture);

            int2 cropSize = new int2((int)spriteRect.width, (int)spriteRect.height);

            var src = texture.GetPixelData<Color32>(0);
            var dst = new NativeArray<Color32>(cropSize.x * cropSize.y, Allocator.TempJob);

            var job = new BlitJob()
            {
                srcPixelData = src,
                dstPixelData = dst,
                srcTextureSize = new int2(texture.width, texture.height),
                dstTextureSize = cropSize,
                srcPos = new int2((int)spriteRect.position.x, (int)spriteRect.position.y),
                spriteRect = spriteRect,
            };

            job.Schedule(cropSize.x * cropSize.y, 32).Complete();

            var bytes = ImageConversion.EncodeNativeArrayToPNG<Color32>(dst, UnityEngine.Experimental.Rendering.GraphicsFormat.R8G8B8A8_UNorm, (uint)cropSize.x, (uint)cropSize.y);// texture.EncodeToPNG();

            File.WriteAllBytes(assetPath, bytes.ToArray());
            ...
            TextureImporterSettings importerSettings = new TextureImporterSettings();
            importer.ReadTextureSettings(importerSettings);
            importerSettings.spritePivot = spritePivot / spriteRect.size;
            importerSettings.spriteAlignment = (int)SpriteAlignment.Custom;
            importer.SetTextureSettings(importerSettings);
        ...
        public static bool IsTextureValidProtoSprite(Texture2D texture, out string invalidReason, out bool isAutoFixable)
        {
            isAutoFixable = false;

            invalidReason = "NULL texture.";
            if (texture == null)
                return false;
        ...
        public static bool IsSpriteValidProtoSprite(Sprite sprite, out string invalidReason, out bool isAutoFixable)
        {
            isAutoFixable = false;
        ...
        public static bool IsSelectionValidProtoSprite(out string invalidReason, out bool isAutoFixable)
        {
            ...
            // Check secondary textures
            var secondaryTextures = importer.secondarySpriteTextures;

            foreach (var st in secondaryTextures)
            {
                if (!IsTextureValidProtoSprite(st.texture, out invalidReason, out isAutoFixable))
                {
                    invalidReason = "(Secondary Texture: " + st.name + ") " + invalidReason;
                    return false;
                }
            }
