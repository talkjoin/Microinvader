    ...
    public class SelectTool : ProtoSpriteTool
    {
        ...
        void UpdateTarget()
        {
            ...
            if (validSelection)
            {
                ...
                selectedTexture = ProtoSpriteWindow.GetTargetTexture();
            }

            bool changedTarget = m_TargetSprite != selectedSprite || m_TargetTexture != selectedTexture;
        ...
        void DrawHoverImageFinal(Sprite sprite)
        {
            ...
            var job = new DrawJob()
            {
                ...
                targetPixelData = m_TargetTexture.GetPixelData<Color32>(0),
                hoverTextureSize = new int2(m_SomeTex.width, m_SomeTex.height),
                targetTextureSize = new int2(m_TargetTexture.width, m_TargetTexture.height),
                hoverTexturePosition = new int2((int)m_SelectionRect.position.x, (int)m_SelectionRect.position.y),
                spriteRect = sprite.rect
            };

            job.Schedule(m_TargetTexture.width * m_TargetTexture.height, 32).Complete();
        ...
        void DrawHoverImagePreview(Sprite sprite)
        {
            ...
            Texture2D texture = m_TargetTexture;// sprite.texture;
            Rect spriteRect = sprite.rect;
        ...
        [BurstCompile]
        public struct DrawJob : IJobParallelFor
        {
        ...
        void CopyIntoHoverTexture(Sprite sprite, Rect rect, bool clearSrc)
        {
            ...
            m_SomeTex = Instantiate(m_TargetTexture);
            ...
            var job = new CopyJob()
            {
                srcPixelData = m_TargetTexture.GetPixelData<Color32>(0),
                dstPixelData = m_SomeTex.GetPixelData<Color32>(0),
                srcTextureSize = new int2(m_TargetTexture.width, m_TargetTexture.height),
