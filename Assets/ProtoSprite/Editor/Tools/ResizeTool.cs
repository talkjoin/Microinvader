...
namespace ProtoSprite.Editor
{
    public class ResizeTool : ProtoSpriteTool
    {
        ...
        void Finish(SpriteRenderer spriteRenderer)
        {
            ...
            // UNDO
            UndoDataResizeTextureGroup undoDataGroup = new UndoDataResizeTextureGroup();

            List<UndoDataResizeTexture> undoCommands = new List<UndoDataResizeTexture>();

            UndoDataResizeTexture undoDataMain = new UndoDataResizeTexture();
            undoDataMain.texture = texture;
            undoDataMain.textureSizeBefore = new Vector2Int(texture.width, texture.height);
            undoDataMain.pixelDataBefore = pixelsBefore;
            undoDataMain.spritePivotNormalizedBefore = sprite.pivot / new Vector2(texture.width, texture.height);

            undoCommands.Add(undoDataMain);

            ProtoSpriteData.SubmitUndoData(undoDataGroup, "ProtoSprite Resize Tool");
            //ProtoSpriteData.SubmitUndoData(undoDataMain, "ProtoSprite Resize Tool");



            // Copy previous pixels into new texture
            DoResize(texture, pixelsBefore, ref pixelsAfter, newTextureWidth, newTextureHeight, spriteRenderer, heightOffset, widthOffset, heightPixelDiff, widthPixelDiff);

            //TextureImporter textureImporter = (TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture));
            SecondarySpriteTexture[] secondarySpriteTextures = textureImporter.secondarySpriteTextures;

            foreach (var sTex in secondarySpriteTextures)
            {
                Texture2D sTex2D = sTex.texture;
                var pixBefore = sTex2D.GetPixels32(0);
                var pixAfter = pixBefore;

                TextureImporter texImporter = (TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(sTex2D));

                TextureImporterSettings impSettings = new TextureImporterSettings();
                texImporter.ReadTextureSettings(impSettings);


                UndoDataResizeTexture undoData = new UndoDataResizeTexture();
                undoData.texture = sTex2D;
                undoData.textureSizeBefore = new Vector2Int(sTex2D.width, sTex2D.height);
                undoData.pixelDataBefore = pixBefore;
                undoData.spritePivotNormalizedBefore = sprite.pivot / new Vector2(sTex2D.width, sTex2D.height);
                //ProtoSpriteData.SubmitUndoData(undoData, "ProtoSprite Resize Tool");
                undoCommands.Add(undoData);


                DoResize(sTex2D, pixBefore, ref pixAfter, newTextureWidth, newTextureHeight, spriteRenderer, heightOffset, widthOffset, heightPixelDiff, widthPixelDiff);

                ProtoSpriteData.SaveData saveData = new ProtoSpriteData.SaveData(sTex2D);
                ProtoSpriteData.SubmitSaveData(saveData);
                ProtoSpriteData.Saving.SaveTextureIfDirty(sTex2D, false);


                impSettings.spriteAlignment = (int)SpriteAlignment.Custom;
                impSettings.spritePivot = pivotNormalized;
                texImporter.SetTextureSettings(impSettings);

                //AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(sTex2D)).SaveAndReimport();


                texImporter.SaveAndReimport();

                // UNDO
                undoData.textureSizeAfter = new Vector2Int(sTex2D.width, sTex2D.height);
                undoData.pixelDataAfter = pixAfter;
                undoData.spritePivotNormalizedAfter = sprite.pivot / new Vector2(sTex2D.width, sTex2D.height);
            }
...

            {
                ...
                // UNDO
                undoDataMain.textureSizeAfter = new Vector2Int(texture.width, texture.height);
                undoDataMain.pixelDataAfter = pixelsAfter;
                undoDataMain.spritePivotNormalizedAfter = sprite.pivot / new Vector2(texture.width, texture.height);

                //undoDataGroup.texture = texture;
                undoDataGroup.commands = undoCommands.ToArray();

            }
        }

        void DoResize(Texture2D texture, Color32[] pixelsBefore, ref Color32[] pixelsAfter, int newTextureWidth, int newTextureHeight, SpriteRenderer spriteRenderer, float heightOffset, float widthOffset, int heightPixelDiff, int widthPixelDiff)
        {
            if (texture.width != newTextureWidth || texture.height != newTextureHeight)
            {
                int oldWidth = texture.width;
